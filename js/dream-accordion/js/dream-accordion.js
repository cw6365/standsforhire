	...
	function initAll(i){
		...
		for(i; i <playlistLength; i++){
			...
			if(!isOfline && !isIEbelow9 && img.length>0){
				...
				if(/http/.test(img_src)){	
				//TO DO: test if same server or cross domain!
	...
	*/
	...
	function hidePreviousSlideData(id) {//hide vertical title and captions
		...
		if(hideTitleOnOpen){
			...
			if(title){
				title.css('display', 'block').stop().animate({'opacity': 1},  {duration: 500, easing: "easeOutQuart"});
			}
		}
